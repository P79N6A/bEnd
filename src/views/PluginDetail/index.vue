...
export default {
    ...
    computed: {
        canUninstall() {
            const relAddPlugins = this.pluginsData.relAddPlugins;
            if (!relAddPlugins.length) {
                return true;
            }
            return !relAddPlugins.some(plugin => Number(plugin.isAdd) === 1);
        },
        canInstall() {
    ...
    methods: {
        ...
        onInstall() {
            // 调用安装钩子
            this.xz.onInstall(this.pluginsData.routerName).then(() => {
